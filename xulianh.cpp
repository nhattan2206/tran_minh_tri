/*
class Image : pixels, width, height, graylevels, transform ...
class Filter : maxtrix, width, height, ...
*/
...
string filepathIn = "D:\\lena-ascii.pgm"; 
...
void Image::WriteFile(string filepath,Filter filter) {
	fstream output; 
	output.open(filepath, ios::out | ios::trunc); 
...
void Filter::HistogramEqua(Image image) {
	...
	int histogram[256];
	// Initialize all the histogram values equal to 0 first;
	for (int i = 0; i < 256; i++) {
		histogram[i] = 0;
	}
	...
	// Compute Cumulative Distributive Function (CDF)
	/* Cong don tan suat cua muc xam hien tai voi tan suat cua muc xam truoc do */
	...
	//Scale the histogram
	/*Tinh ra diem xam moi bang cach lay gia tri CDF * 255*/
	int Scale[256];
	for (int i = 0; i < 256; i++) {
		Scale[i] = round((double)cumHistogram[i] * 255);
	}

	//Generate the equilized matrix
